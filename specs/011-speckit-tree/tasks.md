...
- [x] T001 Add `everReportedStories: boolean` to `Session` in `apps/board/src/main/store.ts`, set by an OR in `putReport` when the incoming report carries at least one story. **Never cleared** — no reset, no else branch, no timer. Say in the comment that reports replace wholesale, so the report on screen cannot answer "has this session ever had stories", which is why the fact is held rather than derived
- [x] T002 **Argue the decision 26 exception in `putReport`'s own comment**, because this is the first thing in that method that survives a replace and the next feature to want one will cite it. The comment must name what it costs — a dropped story-carrying push leaves the flag unset with nothing to correct it — and why it is accepted anyway: this is a derived boolean rather than reported content; it is monotonic, so there is one wrong state rather than progressive drift, and any later push carrying stories heals it; and the failure is benign, since the developer keeps the views they already had. Do **not** paraphrase this from [research.md §2](research.md) — a reader of `store.ts` must be able to answer a second proposal without leaving the file
- [x] T003 [P] Extend `apps/board/tests/unit/store.test.ts` — a report with stories sets it; **ten subsequent reports carrying none leave it set**, which is FR-003 as a unit fact; a session that has never reported stories has it false; dismissal takes it with the session. The middle assertion is the one that matters, and it is the same shape as feature 010's stickiness test. Also assert the monotonicity directly — there is **no** sequence of reports that returns it to false — since that is the property the decision 26 argument rests on
- [x] T004 Add `everReportedStories` to `SessionView` in `apps/board/src/main/view.ts`, listed field by field rather than spread. Leave `SessionSummary` **unchanged** — which destinations a session offers is not a pill's business
- [x] T005 [P] Extend `apps/board/tests/unit/view.test.ts` — the boolean projects as held, and `SessionSummary` still carries nothing about it
- [x] T006 Create `apps/board/src/renderer/src/state/usePlacement.ts` — the rule in [contracts/placement.md](contracts/placement.md), importing `Scope`, `UNASSIGNED` and `scopeKey` from `state/useSelection.ts` as values. `storyId` first and authoritative, `taskIds` only for a task naming no story, everything else into a final unassigned scope that is **absent when empty**. Name `buildScopes` in the comment and say what differs and why
- [x] T007 [P] Write `apps/board/src/renderer/src/state/usePlacement.test.ts` — **`tasks drawn = tasks reported` first**, over a report built to hit every branch at once. Then one case per row of [quickstart.md §4](quickstart.md)'s table. The disagreement row is the one a plausible implementation gets wrong, because `buildScopes` deliberately draws it twice. Teeth-check by switching the rule to a union and confirming the count assertion fails
- [x] T008 Create `apps/board/src/renderer/src/state/usePresence.ts` — the two one-way booleans of [contracts/presence.md](contracts/presence.md): `tree` from `everReportedStories`, `oldViews` from `!tree || usedOldViews.has(sessionKey)`. `usedOldViews` is a `Set` of session ids held here, added to and **never removed from**. Write the one-way property in the file as the reason FR-007 needs no code of its own
- [x] T009 [P] Write `apps/board/src/renderer/src/state/usePresence.test.ts` — every sequence in [contracts/presence.md](contracts/presence.md), and then the property that stands in for FR-007: over any sequence of reports and view-openings, **no destination that was offered becomes un-offered**. Assert it as a property over a sequence, not as a state at one moment
- [x] T010 [P] ~~Create `apps/board/src/renderer/src/lib/progress.ts`~~ — **built in two existing files instead, and the plan was wrong to name a new one.** `tally` went into `lib/vocabulary.ts` because `source-hygiene.test.ts` fails the build if anything outside that file and `StatusLabel.tsx` classifies a status, and counting *is* classifying — the test caught this rather than a reviewer. `storyProgress` and `formatStoryProgress` went into `lib/summarise.ts`, whose own comment argues against scattering counts: it already holds `taskSummary`, and "three components counting it three times is three chances to count it differently". Counts, never a word, and computed only for a story with no reported status of its own
- [x] T011 [P] Write the counting tests in `apps/board/src/renderer/src/lib/summarise.test.ts` (not a new file, per T010) — mixed states counted correctly; `donee`, `almost` and `WIP-ish` counted as unrecognised and **not** as todo or done; every task accounted for exactly once; a story with no tasks yields nothing to show; and no synonym added to the vocabulary
- [x] T012 Create `apps/board/src/renderer/src/state/useTreeState.ts` — selection as `{ kind, id }` intent resolved on render with **no fallback to the first item** (FR-034, and the deliberate divergence from `resolve`), plus expansion as `defaultOpen XOR toggled`, where the default is open for the story holding `focus.task`. No effect, nothing initialised from a report
...
- [x] T014 Add a comment to `apps/board/src/renderer/src/state/useSelection.ts` naming `usePlacement` and stating that a second rule exists, which view uses it, and why this one is unchanged. **No logic change in this file** — a diff here that is not a comment is a mistake
