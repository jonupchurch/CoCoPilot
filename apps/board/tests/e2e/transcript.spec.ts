...
test.describe('unavailable is not empty', () => {
  ...
  test('distinguishes a session with no prompts from one it could not read', async () => {
    ...
    // differently. An unreadable source is the one that means something is
    // wrong, so it carries the attention treatment and the other does not.
    ...
    const emptyColour = await emptyState.evaluate((el) => getComputedStyle(el).backgroundColor);
    ...
    const failedColour = await failed.evaluate((el) => getComputedStyle(el).backgroundColor);

    expect(failedColour).not.toBe(emptyColour);
  });
});

test.describe('the blast radius is three sections', () => {
  test('leaves every reported section working when the transcript is unreadable', async () => {
    install('garbage.jsonl', 'session-1');
    board = await launchBoard({ home });
    ...
    // This is the property the whole dependency rests on: a format we do not own
    // changing must cost three sections and nothing else.
    await expect(board.page.getByTestId('focus-task')).toHaveText('T-013');
    await expect(board.page.getByTestId('section-summary-spec')).toHaveText('US-002 · 1 of 2 done');
    await expect(board.page.getByTestId('section-summary-plan')).toHaveText('Step 1 of 1');
    await expect(board.page.getByTestId('changed-0')).toContainText('src/hooks/useSession.ts');
    await expect(board.page.getByTestId('identity')).toBeVisible();
