...
export const NOT_A_REPOSITORY =
  'Not inside a git repository — CoCoPilot reports are per repository, so there is nothing to report against.';

/** Delivery succeeded. Deliberately says nothing the caller has to act on. */
export const DELIVERED = 'Reported to the CoCoPilot board.';
...
export function versionMismatch(clientContract: string, boardContract: string): string {
  return (
    `CoCoPilot version mismatch: this client speaks contract ${clientContract}, ` +
...
export function rejected(field: string, message: string): string {
  return `CoCoPilot rejected this report: ${field} — ${message}`;
}
